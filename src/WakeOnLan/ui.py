...
<screen position="center,center" size="560,400" title="Wake-On-LAN Configuration" >
    ...
    def __init__(self, session, args=0):
        ...
        self.skinName = ["WakeOnLanConfig", "Setup"]
    ...
    def doremove(self):
        ...
        if not current:
            return
        self.menuitems = [item for item in self.menuitems if item[1] != current[1]]
        self["config"].setList(self.menuitems)

    def save(self):
        try:
            with open(wol.WOLLIST, 'w') as handle:
                for item in self.menuitems:
                    handle.write('%s\n' % item[1])
